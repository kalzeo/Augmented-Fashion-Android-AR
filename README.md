...
7. The `android_sdk_repository` and `android_ndk_repository` paths will need to be configured in the WORKSPACE file found in the root directory of the project. Run the following script to set these up: `sh setup_workspace_sdk_ndk_paths.sh`. This step will only need to be completed once.
